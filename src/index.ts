...
class Player {
    ...
    constructor(name: string, points: number, rounds: number[], color: string, picker: string) {
        this.name = name;
        this.points = points;
        this.rounds = rounds;
        this.color = color;
    ...
    removePoint() {
        if (this.points === 5) this.points = 3;
        else if (this.points === 0) this.points = 0;
        else this.points -= 1;
    }
...
const addPointToPlayer = (pointWinner: number) => {
